...
/cad/mentor/qformal/linux_x86_64/lib/mpiexec -n 1 /cad/mentor/qformal/linux_x86_64/bin/qverifyfk -od Output_Results -tool prove -init test.init -rtl_init_values -effort unlimited -import_db Output_Results/formal_compile.db -mode slave -slave_id 1 -mpiport 'tag#0$port#37719$description#rsg9.stanford.edu$ifname#172.24.74.107$' < /dev/null &
PID=$!
wait ${PID}
wait ${PID}
if [ "$?" -ne "0" ]; then 
  /cad/mentor/qformal/linux_x86_64/lib/mpiexec -n 1 /cad/mentor/qformal/linux_x86_64/bin/qverifyfk -od Output_Results -tool prove -init test.init -rtl_init_values -effort unlimited -import_db Output_Results/formal_compile.db -mode slave -slave_id 1 -mpiport 'tag#0$port#37719$description#rsg9.stanford.edu$ifname#172.24.74.107$' -monitor < /dev/null
fi
